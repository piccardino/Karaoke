class KaraokeVisualizer {
    constructor(container) {
        ...
        this.frequencies = new Array(64).fill(0);
    ...
    updateFrequencies() {
        if (this.isAudioConnected && this.analyser) {
            ...
            for (let i = 0; i < this.frequencies.length && i < this.frequencyData.length; i++) {
                this.frequencies[i] = this.frequencyData[i] / 255;
            }
    ...
    animate() {
        ...
        if (this.particles) {
            ...
            const beat = this.frequencies[0] + this.frequencies[1];
        ...
        if (this.bars) {
            ...
            for (let i = 0; i < this.bars.count; i++) {
                ...
                matrix.makeTranslation(pos.x, pos.y + height / 2, pos.z);
                this.bars.setMatrixAt(i, matrix);
            }
            this.bars.instanceMatrix.needsUpdate = true;
